...
WoWGoldGambler.EXCHANGE.startRolls = function(self)
    ...
    self:ChatMessage("Registration has ended. Let's see which players will be making an exchange!")
...
WoWGoldGambler.EXCHANGE.recordRoll = function(self, playerName, actualRoll, minRoll, maxRoll)
    ...
    if (self.session.dealer.name == playerName and self.session.modeData.firstPlayerIndex == nil and
        ...
        self:ChatMessage(firstPlayerName .. ", you have been selected to make an exchange! Now, /roll " .. #self.session.players .. " to determine your opponent!")
    elseif (self.session.modeData.firstPlayerName == playerName and self.session.modeData.secondPlayerIndex == nil and
        ...
        if (secondPlayerIndex ~= self.session.modeData.firstPlayerIndex) then
            ...
            self:ChatMessage("Alright, " .. playerName .. " and ".. secondPlayerName .. " are about to make an exchange! Here are the possible outcomes!")
            self:ChatMessage("{Skull} " .. playerName .. " owes " .. secondPlayerName .. " " .. self:formatInt(self.db.global.game.wager) .. " gold")
            self:ChatMessage("{Cross} " .. playerName .. " owes " .. secondPlayerName .. " a rolled amount of gold")
            self:ChatMessage("{Circle} " .. playerName .. " and " .. secondPlayerName .. " must /hug")
            self:ChatMessage("{Diamond} " .. secondPlayerName .. " owes " .. playerName .. " a rolled amount of gold")
            self:ChatMessage("{Star} " .. secondPlayerName .. " owes " .. playerName .. " " .. self:formatInt(self.db.global.game.wager) .. " gold")

            self:cycleRaidIcon(true)

            self:ChatMessage("Now, " .. playerName .. ", notice the raid icons cycling above my head. When you're ready, type STOP in chat to determine the outcome!")
        else
            self:ChatMessage("You can't play against yourself! Let's try that again. " .. playerName .. ", /roll ".. #self.session.players .. " to determine your opponent!")
        end
...
WoWGoldGambler.EXCHANGE.handleChatMessage = function(self, text, playerName, playerRealm)
    ...
    if (self.session.modeData.firstPlayerName == playerName and self.session.modeData.loserName == nil and string.upper(text) == "STOP") then
        ...
        if (raidIcon == 1) then
            ...
            self:ChatMessage("{Star} How lucky! It looks like " .. self.session.modeData.loserName .. " will be donating the full wager amount!")
        elseif (raidIcon == 2) then
            -- Circle Outcome
            self.session.modeData.amountOwed = 0
            self.session.modeData.raidIcon = "{Circle}"
            self:ChatMessage("{Circle} Aww, it looks like " .. self.session.modeData.firstPlayerName .. " and ".. self.session.modeData.secondPlayerName .. " have to exchange hugs!")
        elseif (raidIcon == 3) then
            -- Diamond Outcome
            self.session.modeData.loserName = self.session.modeData.secondPlayerName
            self.session.modeData.raidIcon = "{Diamond}"
            self:ChatMessage("{Diamond} It looks like " .. self.session.modeData.loserName .. " is feeling generous! ".. self.session.modeData.loserName .. ", /roll " .. self.db.global.game.wager .. " now to see how much you'll lose!")
        elseif (raidIcon == 7) then
            ...
            self:ChatMessage("{Cross} It looks like " .. self.session.modeData.loserName .. " is feeling generous! ".. self.session.modeData.loserName .. ", /roll " .. self.db.global.game.wager .. " now to see how much you'll lose!")
        elseif (raidIcon == 8) then
            -- Skull Outcome
            self.session.modeData.loserName = self.session.modeData.firstPlayerName
            self.session.modeData.amountOwed = self.db.global.game.wager
            self.session.modeData.raidIcon = "{Skull}"
            self:ChatMessage("{Skull} How generous! It looks like " .. self.session.modeData.loserName .. " will be donating the full wager amount!")
        end
